...
const StatusTransitionModal: React.FC<StatusTransitionModalProps> = ({
  ...
  // Check if file upload is required for DO users closing/resolving tickets
  const isFileUploadRequired = (): boolean => {
    return user?.role === 'DO' && 
           (selectedStatus === 'COMPLETED' || selectedStatus === 'RESOLVED' || selectedStatus === 'CLOSED');
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isFileUploadRequired() && (!uploadedFiles || uploadedFiles.length === 0)) {
      alert('File upload is mandatory for department officers when closing/resolving tickets. Please upload at least one file.');
      return;
    }
      ...
      <div className="flex min-h-screen items-center justify-center p-4">
        ...
        <div className="relative bg-white rounded-lg shadow-2xl max-w-md w-full border-2 border-blue-200">
          ...
          <form onSubmit={handleSubmit} className="p-6">
            ...
            {/* File Upload Section - Mandatory for DO users when closing/resolving */}
              ...
              <div className="mb-6">
                ...
                {isFileUploadRequired() && (!uploadedFiles || uploadedFiles.length === 0) && (
                  <p className="text-sm text-red-600 mt-2 flex items-center space-x-1">
                    ...
                    <span>At least one file must be uploaded before closing/resolving this ticket.</span>
                  </p>
                )}
              </div>
            )}
